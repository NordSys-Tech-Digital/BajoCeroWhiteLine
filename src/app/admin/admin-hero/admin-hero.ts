...
export class AdminHeroComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.form.patchValue({ badge: h.badge, headline: h.headline, headlineHighlight: h.headlineHighlight, subtext: h.subtext });
    h.stats.forEach(s => this.statsArray.push(this.fb.group({ value: [s.value, Validators.required], label: [s.label, Validators.required] })));
  }

  addStat() { this.statsArray.push(this.fb.group({ value: ['', Validators.required], label: ['', Validators.required] })); }
  removeStat(i: number) { if (this.statsArray.length > 1) this.statsArray.removeAt(i); }
